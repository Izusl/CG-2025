...
namespace Canis
{
    ...
    void World::ReplaceEntityTexture(const std::string& tag)
{
    for (size_t i = 0; i < m_entities.size(); ++i)
    {
        if (m_entities[i].tag == tag)
        {
            // Copy existing entity
            Canis::Entity updatedEntity = m_entities[i];

            // Replace texture
            //updatedEntity.albedo = newTexture;

            // Delete old one
            m_entities.erase(m_entities.begin() + i);

            // Add new one
            //m_entities.push_back(updatedEntity);
            return;
        }
    }
}
